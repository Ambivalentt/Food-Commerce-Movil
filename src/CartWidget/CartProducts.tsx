...
const CartProducts = () => {
    const { cartProducts, totalPriceFinal } = useCart() as CartContextProps;
    const { user } = useAuth()
    return (
        <section className="max-w-sm w-full px-2 mx-auto my-5">
            ...
            {cartProducts.length > 0 && (
                <div className="w-full flex items-end pb-10 h-20 flex-col">
                    <button className=" disabled:opacity-60 bg-gray-300 w-40 text-center rounded-xl font-bold text-gray-900"
                        disabled={user === null}>
